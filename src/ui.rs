...
impl UI {
    ...
    pub fn render(&mut self, buffer: &Buffer, mode: &EditorMode, command_line: &str, message: &Option<String>) -> Result<()> {
...
fn render_editor_content(f: &mut ratatui::Frame, area: Rect, buffer: &Buffer, viewport_start: usize, _viewport_height: usize) {
    ...
    for line_num in viewport_start..end_line {
        ...
        let mut spans = vec![
            Span::styled(line_number, Style::default().fg(RatatuiColor::DarkGray))
        ];
...
fn render_status_line(f: &mut ratatui::Frame, area: Rect, buffer: &Buffer, mode: &EditorMode) {
    ...
    let status_line = Paragraph::new(status_content)
        .style(Style::default()
...
fn render_command_line(f: &mut ratatui::Frame, area: Rect, mode: &EditorMode, command_line: &str, message: &Option<String>) {
